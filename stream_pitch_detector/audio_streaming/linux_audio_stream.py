...
class LinuxAudioStream(AudioStream):
    ...
    def _get_default_sink_monitor(self) -> str:
        ...
        A monitor source is a virtual input source that represents the audio output of a sink.
        Since parec requires an input source to record audio, we use the monitor source of the default sink.
    ...
    def start_stream(self):
        ...
        if not self.stream_process:
            self.stream_process = subprocess.Popen(
                self.parec_command, stdout=subprocess.PIPE, bufsize=self.chunk_size
            )
